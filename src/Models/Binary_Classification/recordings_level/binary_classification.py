...
def create_tr_tst_dfs(tr_set:str, tr_path:str, tst_set:str, tst_path:str,class1:int,class2:int,class1_name:str,class2_name:str):
    list_of_dataframes = []
    for f in tst_set:
    ...
    tst_df = tst_df.loc[(tst_df['Label'] == class1 ) | (tst_df['Label'] == class2)]
    ...
    tr_df = tr_df.loc[(tr_df['Label'] == class1 ) | (tr_df['Label'] == class2)]

    #Show the training set class distribution
    show_class_distribution(tr_df,class1,class2,class1_name,class2_name)
    ...
    if target_count[class1] != target_count[class2]:
        os = RandomUnderSampler(sampling_strategy={class1:min_n_samples, class2:min_n_samples})
...
if __name__ == '__main__':
    ...
    tr_path, tst_path = mf.get_train_test_set()
    tr_set = os.listdir(tr_path)
    tst_set = os.listdir(tst_path)

    X_train, y_train, X_test, y_test = create_tr_tst_dfs(tr_set, tr_path, tst_set, tst_path, class1, class2, class1_name, class2_name)
