...
##############################################################
### 1.Creating video-audio folder in the current directory ###
##############################################################

#superuser #enabling super user mode 
current_dir=`pwd` #Saving the current directory in current_dir variable
printf "\nCurrent directory is $current_dir"

folder_name="/video_audio" #The folder where audio and video is going to be saved
new_path=$current_dir$folder_name #Concatenating current working directory and the video audio folder

#Checkig for video_audio folder existance
if [ -d "$new_path" ]; then
    echo "video_audio folder exists..."
else
    ### Create video_audio directory ###
    echo
...
### -the latest version of v4l-utils                     ###
############################################################
dependencies 
